...
int Algorithm::getClientsConnected ( vector<Shapes *> &shapes, 
        vector<int> serverLoc, vector<int> clientLoc )
{
    int server, number; 
    ...
    for ( int n=0; n < serverLoc.size(); n++ )
    {
        ...
        while ( !path.empty() ) 
        {
            ...
            path.erase ( remove ( path.begin (), path.end (), number ), 
                    path.end () ); 
    ...
    return numClientsConnected ( clientLoc, clientList );
}
...
void Algorithm::findPath ( int number, vector<Shapes *> &shapes, 
        vector<int> &clientList, vector<int> &path, vector<int> discardLst )
{
    ...
    {
            if ( shapes.at ( number - 1 )->left &&
                    shapes.at ( left - 1 )->right )
            {
        ...
        {
            if ( shapes.at ( number - 1)->right &&
                    shapes.at ( right - 1)->left )
            {
        ...
        {
            if ( shapes.at ( number - 1 )->bottom && 
                    shapes.at ( bottom - 1 )->up )
            {
...
int Algorithm::numClientsConnected ( vector<int> lst1, vector<int> lst2 )
{
    int num = 0;
