        ...
        switch ( screen->number )
        {
            case 1:               
                ...
                if ( snd->screen != 1 )
                {
                    ...
                    pthread_create ( &sndThread, NULL, playSound, 
                            ( void * ) NULL ); 
                }                
                break;
            ...
            case 2:                
                printCo();
                DrawImage ( bOptions, squares.at ( 0 )->x1 - 270,
                        squares.at ( 0 )->x1 - 5, squares.at ( 0 )->y1, 
                        squares.at ( ( rows - 1 )*cols )->y3 );
                
                if ( WON )
                {
                    DrawImage ( happy, squares.at ( cols - 1 )->x2 + 5, 
                            squares.at ( cols - 1 )->x2 + 355, 400,  650 );
                
                }else
                {    
                    DrawImage ( thinking, squares.at ( cols - 1 )->x2 + 5, 
                            squares.at ( cols - 1 )->x2 + 355, 400, 650 );
                }
...
void checkKeyboard ( bool &go )
{
    ...
    {
        loadLevel ( 1, ai, shapes, clientLoc, serverLoc, leftDistance, 
                btmDistance, cellDim, rows, cols );
        buildGrid();
        screen->number = 2;          
    ...
    {
        loadLevel ( 3, ai, shapes, clientLoc, serverLoc, leftDistance,
                btmDistance, cellDim, rows, cols );
        buildGrid();
        screen->number = 2;  
    ...
    {
        loadLevel ( 5, ai, shapes, clientLoc, serverLoc, leftDistance, 
                btmDistance, cellDim, rows, cols );
        buildGrid();
        screen->number = 2;          
...
void GLFWCALL mouse_click ( int button, int action ) 
{
    ...
    {
        if ( currCell < 1 || currCell > rows*cols || WON || 
                screen->number != 2 )
        {
            return;
        }
        ...
        totalClicks++;
        cout << "Total number of clicks: " << totalClicks << endl;
        cout << "Total number of clients connected: " << totClientsConn << endl;
        ...
        { 
            ...
            pthread_t thread;
            pthread_create ( &thread, NULL, startThread, ( void * ) screen );  
        }
    }
}
...
{    
    ...
    */
    ...
    result = system->createSound ( snd->filename.c_str(), FMOD_SOFTWARE |
